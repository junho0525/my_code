function [D,DCM] = power_ratio_map_neighbor_source(config)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
roiInd = [];
for i= 1:10
    roiInd(end+1) = find(min(tempDistance)==tempDistance);
    tempDistance(roiInd(end)) = max(tempDistance);
end
tmp = [];
for i=1:length(source.trial)
    tmp(end+1) = source.trial(i).pow(roiInd(1));
    for j=2:10
        tmp(end) = source.trial(i).pow(roiInd(j));
    end
end
