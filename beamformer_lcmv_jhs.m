function [source] = beamformer_lcmv_jhs(dip, grad, headmodel, data, Cy, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This code is modified version of beamformer_lcmv in fieldtrip-20180205
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Finally edited                                                          %
...
leadfield = data.leadfield.leadfield;
% if length(dip.inside)<length(data.leadfield.leadfield)
%     leadfield = {data.leadfield.leadfield{dip.inside}};
% elseif length(dip.inside)<length(data.leadfield.leadfield)
%     leadfield = data.leadfield.leadfield;
% else
%     error('leadfield matrix dimension is smaller than the number of mesh points.');
% end
...
%       source = copyfields(dipout, source, {'pos', 'inside', 'leadfield', 'leadfielddimord', 'label'});%, 'filter'});
% 
%       % prevent duplication of these fields when copying the content of dip into source.avg or source.trial
%       dip    = removefields(dipout,       {'pos', 'inside', 'leadfield', 'leadfielddimord', 'label'});%, 'filter'});
% 
%       if istrue(cfg.(cfg.method).keepfilter) && isfield(dipout(1), 'filter')
%         for k = 1:numel(dipout)
%           dipout(k).label        = sens.label;
%           dipout(k).filterdimord = '{pos}_ori_chan';
%         end
%       end
%     end
% 
