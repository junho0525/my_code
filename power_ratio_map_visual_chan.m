function [D,DCM] = power_ratio_map_visual_chan(config)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
chanlabel = {'A238','A239','A164','A165','A186','A185','A187','A203','A204','A220'};
chanind = [];  % find the sensor where power is max
for i = 1:length(chanlabel)
    chanind(end+1) = find(strcmp(datapow.label,chanlabel{i}));
end
tmp = tmp(:,chanind);
tmp = sum(tmp,2);
