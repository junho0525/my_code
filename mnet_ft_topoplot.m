function mnet_ft_topoplot(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% mnet sensor power visualize using ft_freqanalysis and ft_topoplot
% 
% Perform multitaper fourier transform useing ft_freqanalysis and then
% visualize sensor space power spectrum
%
% USAGE
%   mnet_ft_topoplot(data, band, datalabels, plotsize)
...
data = varargin{1};
band = varargin{2};
if iscell(data), ismultisubj=1; else, ismultisubj = 0; end
...
if ismultiband 
        ...
        if ischar(band{i})
            ...
            switch band{i}
                ...
                case 'gamma2'
                    bandfreq{i} = [60 90];
                case 'all'
                    if isfield(data{1},'data')
                        bandfreq{i} = [0.1 data{1}.data.fsample/2]
                    else
                        bandfreq{i} = [0.1 data{1}.fsample/2];
                    end
                otherwise
                    error('Unexpected band name');
            end
        end
    end
else
    ...
    if ischar(band)
        ...
        switch band
            ...
            case 'all'
                if isfield(data{1},'data')
                    bandfreq = [0.1 data{1}.data.fsample/2];
                else
                    bandfreq = [0.1 data{1}.fsample/2];
                end
        end
    end
end
