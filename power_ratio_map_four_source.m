function [D,DCM] = power_ratio_map_four_source(config)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if exist(fullfile(config.savePath,['source_' config.subjectID '_' config.sessionID '.mat']), 'file') ~= 0
    load(fullfile(config.savePath,['source_' config.subjectID '_' config.sessionID '.mat']));
else
    cfg                   = [];
    ...
    save(fullfile(config.savePath,['source_' config.subjectID '_' config.sessionID]));
end
%% identify the indices of trials with high and low alpha power
use_my_library('ft',0);
use_my_library('spm',1);

tempDistance = D.inv{1}.mesh.tess_mni.vert - repmat([-46,-66,30],size(D.inv{1}.mesh.tess_mni.vert,1),1);
tempDistance = sum(tempDistance.^2,2);
roiInd(1) = find(min(tempDistance)==tempDistance);
tempDistance = D.inv{1}.mesh.tess_mni.vert - repmat([49,-63,33],size(D.inv{1}.mesh.tess_mni.vert,1),1);
tempDistance = sum(tempDistance.^2,2);
roiInd(2) = find(min(tempDistance)==tempDistance);
tempDistance = D.inv{1}.mesh.tess_mni.vert - repmat([0, -58, 0],size(D.inv{1}.mesh.tess_mni.vert,1),1);
tempDistance = sum(tempDistance.^2,2);
roiInd(3) = find(min(tempDistance)==tempDistance);
tempDistance = D.inv{1}.mesh.tess_mni.vert - repmat([-1,54,27],size(D.inv{1}.mesh.tess_mni.vert,1),1);
tempDistance = sum(tempDistance.^2,2);
roiInd(4) = find(min(tempDistance)==tempDistance);
tmp = [];
for i=1:length(source.trial)
    tmp(end+1) = 0;
    for j =1:4
        tmp(end) = tmp(end) + source.trial(i).pow(roiInd(j));
    end
end
